"""
Robust RAG service with LOCAL MODEL support - CHATPROMPT INTEGRATION
...
# ChatPromptTemplate: try common import paths
try:
...
# Text fallback template (keeps same instructions used previously)
...
# Try to create ChatPromptTemplate if available
if ChatPromptTemplate is not None:
    try:
        ...
        # if that signature isn't supported, leave None and fallback to text_template
        chat_prompt_obj = None
...
def create_rag_chain(llm, vs):
    ...
    try:
        ...
        if chat_prompt_obj is not None:
            # prefer chat-style template (LangChain will use this safely)
            try:
...
def build_fallback_prompt(context: str, question: str) -> str:
    header = "You are a cybersecurity expert. Use the provided context to answer accurately and clearly. If info is missing, say you cannot find it in the context."
    ctx = context.strip() if context and context.strip() else "(no context available)"
    footer = "\n\nPlease answer concisely and follow the instruction: if the question asks to list or enumerate, list unique items; if it asks to explain, provide a 3-5 sentence explanation. Use only context."
    return f"{header}\n\nContext:\n{ctx}\n\nQuestion:\n{question}\n{footer}\n\nAnswer:"
...
async def generate_text(req: QueryRequest):
    ...
    # Manual flow: retrieve context and format ChatPromptTemplate (or fallback)
    vs = ml_models.get("vector_store")
    ...
    # Format prompt: prefer ChatPromptTemplate object if available
    formatted_prompt = None
    if chat_prompt_obj is not None:
        try:
            # ChatPromptTemplate may provide format or format_messages / format_prompt depending on version
            try:
                formatted_prompt = chat_prompt_obj.format(context=context_text, question=req.query)
            except Exception:
                try:
                    # some versions use format_prompt -> .to_string() or similar
                    fp = chat_prompt_obj.format_prompt(context=context_text, question=req.query)
                    # fp might be a PromptValue or similar; try to extract text
    ...
    # Fallback: use plain text template
